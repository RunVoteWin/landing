...
const timeframeOptions = [
  ...
  { id: 'primary-general', label: 'Primary and general' },
];
...
function Pricing() {
  ...
  const timeframeMultiplier = timeframe === 'primary-general' ? 1.75 : 1;
  const monthlyTotal = selectedCampaign ? Math.round(selectedCampaign.price * timeframeMultiplier) : null;
  const currentStep = !selectedCampaign || !timeframe || !state ? 1 : !role ? 2 : 3;
  ...
  async function handlePricingSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await postLead({
        ...
        state,
        timeframe,
        timeframeLabel: timeframeOptions.find((option) => option.id === timeframe)?.label ?? timeframe,
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-6 text-on-surface-variant">
                    Start with the size, timeframe, and state of the race. We will tighten the estimate from there.
                  </p>
                </div>
                <div className="grid gap-3">
                  {campaignOptions.map((option) => (
                    <button
                      ...
                      className={`rounded-md border p-4 text-left transition hover:border-accent hover:shadow-sm ${
                        campaignId === option.id ? 'border-primary bg-primary/8' : 'border-outline-variant bg-white'
                      }`}
                    >
                ...
                {selectedCampaign && (
                  <div className="grid gap-4 rounded-md border border-outline-variant bg-white p-4">
                    <div>
                      <p className="text-sm font-extrabold uppercase text-primary">Campaign timeframe</p>
                          ...
                          <button
                            ...
                            className={`rounded-md border px-3 py-3 text-sm font-bold transition ${
                              timeframe === option.id
                                ...
                                : 'border-outline-variant bg-surface text-primary hover:border-accent'
                            }`}
                          >
                            {option.label}
                          </button>
              ...
              <div className="space-y-5">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-sm leading-6 text-on-surface-variant">
                      <span className="font-bold text-primary">{selectedCampaign.label}</span> · {timeframeOptions.find((option) => option.id === timeframe)?.label} · {state} · {roleOptions.find((option) => option.id === role)?.label}
                    </p>
                  </div>
                  <button
                    type="button"
